...
-   [命令](#commands)
    -   [analyze:分析哪些目标受文件列表影响.](#analyze)
    -   [args:显示或配置构建声明的参数.](#args)
    -   [检查:检查标头依赖项.](#check)
    -   [clean:清除输出目录.](#clean)
    -   [desc:显示有关目标或配置的大量有见地的信息.](#desc)
    -   [格式:格式.gn文件.](#format)
    -   [gen:生成ninja文件.](#gen)
    -   [帮助:你的想法.](#help)
    -   [ls:列出匹配目标.](#ls)
    -   [path:查找两个目标之间的路径.](#path)
    -   [refs:查找引用目标或文件的内容.](#refs)
-   [目标声明](#targets)
    -   [action:声明一次运行脚本的目标.](#action)
    -   [action_foreach:声明在一组文件上运行脚本的目标.](#action_foreach)
    -   [bundle_data:\[iOS / macOS\]声明没有输出的目标.](#bundle_data)
    -   [copy:声明复制文件的目标.](#copy)
    -   [create_bundle:\[iOS / macOS\]构建iOS或macOS包.](#create_bundle)
    -   [executable:声明可执行目标.](#executable)
    -   [group:声明一组命名的目标.](#group)
    -   [loadable_module:声明可加载的模块目标.](#loadable_module)
    -   [shared_library:声明共享库目标.](#shared_library)
    -   [source_set:声明源集目标.](#source_set)
    -   [static_library:声明静态库目标.](#static_library)
    -   [target:使用给定的编程类型声明目标.](#target)
-   [构建文件功能](#functions)
    -   [断言:在生成时断言表达式为真.](#assert)
    -   [config:定义配置对象.](#config)
    -   [declare_args:声明构建参数.](#declare_args)
    -   [defined:返回是否定义了标识符.](#defined)
    -   [exec_script:同步运行脚本并返回输出.](#exec_script)
    -   [foreach:迭代列表.](#foreach)
    -   [forward_variables_from:复制来自不同范围的变量.](#forward_variables_from)
    -   [get_label_info:从目标的标签中获取属性.](#get_label_info)
    -   [get_path_info:提取文件或目录名的部分内容.](#get_path_info)
    -   [get_target_outputs:\[文件列表\]获取目标的输出列表.](#get_target_outputs)
    -   [getenv:获取环境变量.](#getenv)
    -   [import:将文件导入当前范围.](#import)
    -   [not_needed:将范围内的变量标记为不需要.](#not_needed)
    -   [pool:定义池对象.](#pool)
    -   [print:打印到控制台.](#print)
    -   [process_file_template:对文件列表进行模板扩展.](#process_file_template)
    ...
    -   [set_sources_assignment_filter:设置模式以过滤源文件.](#set_sources_assignment_filter)
    -   [split_list:将列表拆分为N个不同的子列表.](#split_list)
...
-   [内置预定义变量](#predefined_variables)
    ...
    -   [host_cpu:\[string\]运行GN的处理器体系结构.](#host_cpu)
    -   [host_os:\[string\] GN正在运行的操作系统.](#host_os)
    -   [invoker:\[string\]模板内的调用范围.](#invoker)
    -   [python_path:\[string\] Python的绝对路径.](#python_path)
    -   [root_build_dir:\[string\]运行构建命令的目录.](#root_build_dir)
    -   [root_gen_dir:\[string\]工具链生成的文件的目录.](#root_gen_dir)
    -   [root_out_dir:\[string\]工具链输出文件的根目录.](#root_out_dir)
    -   [target_cpu:\[string\]构建所需的cpu体系结构.](#target_cpu)
...
-   [您在目标中设置的变量](#target_variables)
    -   [all_dependent_configs:\[标签列表\]配置为强制依赖.](#all_dependent_configs)
    -   [allow_circular_includes_from:\[标签列表\]允许包含来自deps.](#allow_circular_includes_from)
    -   [arflags:\[string list\]传递给static_library archiver的参数.](#arflags)
    -   [args:\[string list\]传递给动作的参数.](#args)
    -   [asmflags:\[string list\]传递给汇编程序的标志.](#asmflags)
    -   [assert_no_deps:\[标签模式列表\]确保这些目标上没有deps.](#assert_no_deps)
    -   [bundle_contents_dir:在create_bundle中扩展{{bundle_contents_dir}}.](#bundle_contents_dir)
    -   [bundle_deps_filter:\[标签列表\]过滤掉的标签列表.](#bundle_deps_filter)
    -   [bundle_executable_dir:在create_bundle中扩展{{bundle_executable_dir}}](#bundle_executable_dir)
    -   [bundle_plugins_dir:在create_bundle中扩展{{bundle_plugins_dir}}.](#bundle_plugins_dir)
    -   [bundle_resources_dir:在create_bundle中扩展{{bundle_resources_dir}}.](#bundle_resources_dir)
    -   [bundle_root_dir:在create_bundle中扩展{{bundle_root_dir}}.](#bundle_root_dir)
    -   [cflags:\[string list\]传递给所有C编译器变体的标志.](#cflags)
    -   [cflags_c:\[string list\]传递给C编译器的标志.](#cflags_c)
    -   [cflags_cc:\[string list\]传递给C ++编译器的标志.](#cflags_cc)
    -   [cflags_objc:\[string list\]传递给Objective C编译器的标志.](#cflags_objc)
    -   [cflags_objcc:\[string list\]传递给Objective C ++编译器的标志.](#cflags_objcc)
    ...
    -   [code_signing_script:\[文件名\]代码签名脚本.](#code_signing_script)
    -   [code_signing_sources:\[文件列表\]代码签名步骤的来源.](#code_signing_sources)
    -   [complete_static_lib:\[boolean\]将所有deps链接到静态库.](#complete_static_lib)
    -   [配置:\[标签列表\]配置应用于此目标或配置.](#configs)
    -   [data:\[文件列表\]运行时数据文件依赖项.](#data)
    -   [data_deps:\[标签列表\]非链接依赖项.](#data_deps)
    -   [定义:\[string list\] C预处理器定义.](#defines)
    -   [depfile:\[string\]操作的输入依赖项的文件名.](#depfile)
    -   [deps:\[标签列表\]私有链接依赖项.](#deps)
    -   [朋友:\[标签模式列表\]允许目标包含私有标头.](#friend)
    -   [include_dirs:\[目录列表\]其他包含目录.](#include_dirs)
    -   [inputs:\[file list\]其他编译时依赖项.](#inputs)
    -   [ldflags:\[string list\]传递给链接器的标志.](#ldflags)
    -   [lib_dirs:\[目录列表\]其他库目录.](#lib_dirs)
    -   [libs:\[string list\]要链接的其他库.](#libs)
    -   [output_dir:\[directory\]输出文件的目录.](#output_dir)
    -   [output_extension:\[string\]用于输出文件扩展名的值.](#output_extension)
    -   [output_name:\[string\]输出文件的名称,不是默认值.](#output_name)
    -   [output_prefix_override:\[boolean\]不要使用前缀作为输出名称.](#output_prefix_override)
    -   [输出:\[文件列表\]操作和复制目标的输出文件.](#outputs)
    -   [partial_info_plist:\[filename\]资产目录编译器中的路径plist.](#partial_info_plist)
    -   [pool:\[string\]操作使用的池的标签.](#pool)
    -   [precompiled_header:\[string\]要预编译的头文件.](#precompiled_header)
    -   [precompiled_header_type:\[string\]"gcc"或"msvc".](#precompiled_header_type)
    -   [precompiled_source:\[文件名\]要预编译的源文件.](#precompiled_source)
    -   [product_type:\[string\] Xcode项目的产品类型.](#product_type)
    -   [public:\[file list\]声明目标的公共头文件.](#public)
    -   [public_configs:\[标签列表\]适用于家属的配置.](#public_configs)
    -   [public_deps:\[label list\]声明公共依赖项.](#public_deps)
    -   [response_file_contents:\[string list\]操作的.rsp文件的内容.](#response_file_contents)
    -   [script:\[文件名\]操作的脚本文件.](#script)
    -   [来源:\[文件列表\]目标的源文件.](#sources)
    -   [testonly:\[boolean\]声明一个目标必须只用于测试.](#testonly)
    -   [visibility:\[label list\]可依赖于目标的标签列表.](#visibility)
    -   [write_runtime_deps:将目标的runtime_deps写入给定路径.](#write_runtime_deps)
    -   [xcode_extra_attributes:\[scope\] Xcode项目的额外属性.](#xcode_extra_attributes)
    -   [test_application_name:\[string\]测试unit或ui测试目标的应用程序名称.](#test_application_name)
-   [其他帮助主题](#other)
    -   [all:立即打印所有帮助](#all)
    -   [buildargs:构建参数如何工作.](#buildargs)
    -   [dotfile:关于toplevel .gn文件的信息.](#dotfile)
    -   [执行:构建图形和执行概述.](#execution)
    -   [语法:GN构建文件的语言和语法.](#grammar)
    -   [input_conversion:处理来自exec_script和read_file的输入.](#input_conversion)
    -   [label_pattern:匹配多个标签.](#label_pattern)
    -   [标签:关于标签.](#labels)
    -   [ninja_rules:如何命名Ninja构建规则.](#ninja_rules)
    -   [nogncheck:注释包含用于检查.](#nogncheck)
    -   [output_conversion:指定如何将值转换为输出.](#output_conversion)
    -   [runtime_deps:运行时依赖项计算的工作原理.](#runtime_deps)
    -   [source_expansion:将源映射到脚本的输出.](#source_expansion)
    -   [开关:显示可用的命令行开关.](#switches)
